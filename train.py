...
comboExample = ttk.Combobox(window, 
                            values=[
                                    '1st Period','2nd Period','3rd Period','4th Period','5th Period','6th Period','7th Period'],state="readonly",width=21,takefocus=True)

comboExample.grid(column=0, row=1)
comboExample.set("Select Period")
#comboExample.current(1)
comboExample.place(x=1100,y=400)
    ...
    if (is_number(Id) and name.isalpha()):
        ...
        while (True):
            ...
            for (x, y, w, h) in faces:
                ...
                # display the frame
                cv2.imshow('Storing your facial details', img)
...
def TrackImages():
    ...
    col_names = ['Id', 'Name', 'Date', 'Time','Period']
    attendance = pd.DataFrame(columns=col_names)
    while True:
        currnetitem = comboExample.current()
        if(currnetitem<0):
                    messagebox.showwarning("Error","Please select Period")
                    messagebox.destroy()
        else:
            ret, im = cam.read()
            ...
            for (x, y, w, h) in faces:
                ...
                if (conf < 90):
                    ...
                    periodnum=comboExample.get()
                
                    aa = df.loc[df['Id'] == Id]['Name'].values
                    tt = str(Id) + "-" + aa
                    attendance.loc[len(attendance)] = [Id, aa, date, timeStamp,periodnum]
    ...
    # row_count = sum(1 for column in reader)
    # res = attendance
    # print("Total no. of rows: %d"%(reader.line_num))
    message2.configure(text="Total Students present :"+str(int(row_count-1)))
...
Readme = tk.Button(window, text="Readme", command=readmee, fg="red", bg="yellow", width=10, height=2,
                       activebackground="Red", font=('times', 15, ' bold '))
Readme.place(x=1100, y=700)
